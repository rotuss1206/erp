...
function erp_crm_get_life_stages_dropdown_raw( $label = [], $counts = [] ) {
    ...
    $life_stages = [
        'customer'    => _n( 'Member', 'Members', $counts['customer'], 'erp' ),
        'lead'        => _n( 'Enquiry', 'Enquiries', $counts['lead'], 'erp' ),
        'opportunity' => _n( 'Trial Member', 'Trial Members',  $counts['opportunity'], 'erp' ),
        'subscriber'  => _n( 'Lead', 'Leads', $counts['subscriber'], 'erp' )
    ];
...
function erp_crm_get_customer_type( $label = [] ) {
    ...
    $type = [
        'customer' => __( 'Member', 'erp' ),
        'company'  => __( 'Organization', 'erp' ),
    ];
...
function erp_crm_get_feed_activity( $postdata ) {
    ...
    if ( current_user_can( 'erp_crm_agent' ) ) {
        ...
        $people_ids = array_keys( $wpdb->get_results( "SELECT id FROM {$wpdb->prefix}erp_peoples WHERE contact_owner = {$contact_owner} or user_id= $contact_owner", OBJECT_K ) );
