    ...
    **/
    public function create_transaction() {
        ...
        $fields = [
            ...
            'billing_address' => '',
            'ref'             => '',
            'issue_date'      => $this->order->order_date,
            'due_date'        => $this->order->order_date,
            'summary'         => '',
        ...
        if ( 'completed' == $order_status ) {
            $transaction_id = get_post_meta( $this->order->id, '_erp_ac_transaction_id', true );
            ...
            $transaction_id    = get_post_meta( $this->order->id, '_erp_ac_transaction_id', true );
            $already_completed = get_post_meta( $this->order->id, '_erp_wc_order_status', true );
            ...
            if ( $already_completed == 'completed' ) {
                $payment_id = get_post_meta( $this->order->id, '_erp_ac_transaction_payment_id', true );
                erp_ac_remove_transaction( $payment_id );
                update_post_meta( $this->order->id, '_erp_ac_transaction_payment_id', '' );
        ...
        if ( 'completed' == $order_status ) {
            update_post_meta( $this->order->id, '_erp_ac_transaction_payment_id', $insert_id );
            update_post_meta( $this->order->id, '_erp_wc_order_status', 'completed' );
        } else {
            update_post_meta( $this->order->id, '_erp_ac_transaction_id', $insert_id );
        }
